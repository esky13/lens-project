...
folder_name = '20250224_withscatter_lenstool'
...
names = []
try:
    file = open(f'../lens-data/RXJ2248/without_prior/{folder_name}/bayes.dat', 'r')
except FileNotFoundError:
    print('no without prior result.')
else:
    lines = file.readlines()
    chain_num = 0
    for line in lines:
    ...
    print(f'without prior: min Chi2 sigma={sigma_mean}, min Chi2 rcut={rcut_mean}')
...
sigma_mean = samples_data['Pot0 sigma (km/s)'][i]
rcut_mean = samples_data['Pot0 rcut (arcsec)'][i]
print(f'with prior: min Chi2 sigma={sigma_mean}, min Chi2 rcut={rcut_mean}')
...
plt.savefig(f'/home/lmx/work/lens-project/{folder_name}.png')
plt.show()
