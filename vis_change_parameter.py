    ...
    try:
        ...
        for i in range(len(df)):
            ...
            if len(gal.gal_id) < 3:
                ...
                v_disp = gal.v_disp*new_sigma0/sigma0
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            if len(gal.gal_id) < 3:
                ...
                cut_radius = gal.cut_radius*new_rcut0/rcut0
            kwargs = {'zl': gal.z_lens,
                    'zs': z_source,
...
def update_vdisp():
    ...
    global df_scatter
    try:
        new_v_disp = float(entry_vdisp1.get())
        dpie_scatter = []
        for i in range(len(df)):
            gal = df.loc[i]
            e = 1-np.sqrt((1-gal.ellipticite)/(1+gal.ellipticite))
        ...
        print('Some error occurs. Or please enter a valid v_disp')
...
data_dir = '/home/lmx/work/lens-data'
# read galaxy and cluster information from this file.
filename = f'{data_dir}/{cluster_name}/origin_data/best.par'
...
ref_RA, ref_DEC = lst.getRef_RA_DEC(f'{data_dir}/{cluster_name}/origin_data/best.par')  
ps_sourcex_lst, ps_sourcey_lst, ps_sourcez_lst = import_sources(f'{data_dir}/{cluster_name}/generate_origin_sources/source.dat')
...
root.title('Lensing Visualization')

# Create a frame for the plot
plot_frame = tk.Frame(root)
plot_frame.grid(row=0, column=0, sticky="nsew")

# Create a frame for the controls (label, entry, button)
control_frame = tk.Frame(root)
control_frame.grid(row=0, column=1, sticky="nsew")

# Add input fields and buttons for zs, v, and r
# zs
label_zs = Label(control_frame, text="Enter zs:")
label_zs.pack()
entry_zs = Entry(control_frame)
entry_zs.insert(0, str(z_source))
entry_zs.pack()
button_zs = Button(control_frame, text="Confirm zs", command=update_zs)
button_zs.pack()

# v
label_v = Label(control_frame, text="Enter sigma0 (km/s):")
label_v.pack()
entry_v = Entry(control_frame)
entry_v.insert(0, str(sigma0))
entry_v.pack()
button_v = Button(control_frame, text="Confirm sigma0", command=update_sigma0)
button_v.pack()

# r
label_r = Label(control_frame, text="Enter rcut0 (arcsecond):")
label_r.pack()
entry_r = Entry(control_frame)
entry_r.insert(0, str(rcut0))
entry_r.pack()
button_r = Button(control_frame, text="Confirm rcut0", command=update_rcut0)
button_r.pack()

# potentiel 1 v_disp
label_vdisp1 = Label(control_frame, text="Enter v_disp (km/s):")
label_vdisp1.pack()
entry_vdisp1 = Entry(control_frame)
entry_vdisp1.insert(0, str(df.loc[0].v_disp))
entry_vdisp1.pack()
button_vdisp1 = Button(control_frame, text="Confirm v_disp", command=update_vdisp)
button_vdisp1.pack()
...
def update_listbox():
    ...
    for i, (x,y,z,shown) in enumerate(zip(ps_sourcex_lst,ps_sourcey_lst,ps_sourcez_lst,ps_shown_lst)):
        listbox.insert(tk.END, f"{'S' if shown == 1 else ' '} {i}, z={z:.2f}, ({x:.3f},{y:.3f})")
    plot_lensing()

label_source = Label(control_frame, text="Sources table:")
# Create a Listbox to display items
listbox = Listbox(control_frame)
listbox.pack()
...
toggle_button = Button(control_frame, text="Show/Hide Selected Source", command=toggle_state)
toggle_button.pack()
enable_all_button = Button(control_frame, text="Show All", command=lambda: set_all_states(1))
enable_all_button.pack()
disable_all_button = Button(control_frame, text="Hide All", command=lambda: set_all_states(0))
